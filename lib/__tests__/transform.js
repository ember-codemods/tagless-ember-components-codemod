...
describe('classic components', () => {
  ...
  `;
    ...
    expect(generateSnapshot(source, template, { hasTestSelectors: true })).toMatchSnapshot();
  });
  ...
  test('handles `hasTestSelectors` option correctly', () => {
    let source = `
    export default Component.extend({
      'data-test-item': 1,
      'data-test-name': 'Zoey',
  ...
  `;
    ...
    expect(generateSnapshot(source, template, { hasTestSelectors: false })).toMatchSnapshot();
  });
